...
namespace platform {
struct uvc_device {
 uint8_t status=0xc0; unsigned writes=0,reads=0; bool fail_get=false,fail_set=false;
 bool get_xu(int unit,uint8_t control,uint8_t *out,int n) {
  assert(unit==3 && control==0x1b && n==1);reads++;*out=status;return !fail_get;
 }
...
struct synthetic_sensor {
 bool opened=false,fail_open=false; unsigned opens=0;
 ...
 void close(){assert(opened);opened=false;}
};
...
int main() {
 ...
 setenv("RS2_GMSL_IMU_BATCH","bad",1);assert(failed(s,requests()));unsetenv("RS2_GMSL_IMU_BATCH");
 ...
 puts("SDK IMU XU: paired/single/legacy/opt-out/invalid/busy/open-failure rollback tests passed");
}
'''
